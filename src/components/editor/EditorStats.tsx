  ...
  if (compact) {
    return (
      <div className={`flex items-center gap-3 text-xs ${className}`}>
        <span className="text-slate-400">{stats.wordCount.toLocaleString()} words</span>
        <span className="text-slate-500">•</span>
        <span className="text-slate-400">{stats.characterCount.toLocaleString()} chars</span>
        <span className="text-slate-500">•</span>
        <span className="text-slate-400">~{stats.readingTime} min</span>
      </div>
    )
  }
